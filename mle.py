...
def main():
    ...
    # Train the model
    weights, losses = train_logistic_regression(X_train, y_train, lr, epochs)
    ...
    plt.figure()
    plt.title('Training Loss over Epochs (MLE)')
    plt.plot(range(epochs), losses, label='Training Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig('mle_training_loss.jpg')
    ...
    disp.plot()
    plt.title("MLE Confusion Matrix")
    plt.savefig("mle_confusion_matrix.jpg")
